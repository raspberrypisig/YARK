...
void Stage::Toggle(std::string option, bool* ptr, XY pos) {
	XY renderAt = lastPos + pos;
	IM::ToggleSwitch(renderAt, option, ptr);
	lastPos.x += TOGGLE_SIZE + 20;
}

/*void Stage::SASOption(uint8_t thisMode, XY pos) {
	XY renderAt = lastPos + pos;
...
*/
/*void Stage::FloatPM(XY pos, std::string label, float* fl, float grad, int dig, bool sign) {
...
void Stage::Draw(XY pos, XY size) 
{
