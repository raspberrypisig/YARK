...
/*void Throttle::Toggle(std::string option, bool* ptr, XY pos) {
	XY renderAt = lastPos + pos;
	IM::ToggleSwitch(renderAt, option, ptr);
	lastPos.x += TOGGLE_SIZE + 20;
}*/

/*void Throttle::SASOption(uint8_t thisMode, XY pos) {
	XY renderAt = lastPos + pos;
...
*/
/*void Throttle::FloatPM(XY pos, std::string label, float* fl, float grad, int dig, bool sign) {
...
std::string Throttle::GetTitle() {
	return "Throttle";
}

void Throttle::Draw(XY pos, XY size)
{
	VesselPacket vp = client.Vessel;

	/*bool b;
	if ((b = vp.GetMainControl(1 << 6)) != oldVessel.GetMainControl(1 << 6))
	{
		bmcstage = b;
	}*/
...
/*	Toggle("Brakes", &mc[4], pos);
	...
	//client.Control.SetMainControl(1 << 6, bmcstage);
